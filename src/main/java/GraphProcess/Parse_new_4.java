...
//    private List<>
    ...
    Parse_new_4(String srcFile) throws FileNotFoundException {
        try {
    ...
    public List<Node> getSliceOfMethod(MethodCallExpr methodCallExpr, MethodDeclaration methodDeclaration) {
        ...
        List<MethodCallExpr> all_methodcalls_of_md = methodDeclaration.findAll(MethodCallExpr.class);
...
//            a.getScope()
        ...
        while (ifMap) {
            ...
            for (MethodCallExpr callExpr: all_methodcalls_of_md) {
                if (methodCallExprs_want.contains(callExpr)){continue;}
                callExpr.getArguments().forEach(call_args -> {
                    List<String> tmp_names = call_args.findAll(NameExpr.class).stream().filter(nameExpr -> !this.SimpleNamesOfClassOrInterfaceType.contains(nameExpr.getName())).map(NameExpr::getNameAsString).collect(Collectors.toList());
                    List<String> tmp_fields = call_args.findAll(FieldAccessExpr.class).stream().filter(fieldAccessExpr -> fieldAccessExpr.getScope().isThisExpr()).map(FieldAccessExpr::getNameAsString).collect(Collectors.toList());
                    for (NameExpr nameExpr: nameExprs_want_done) {
                        if (tmp_names.contains(nameExpr.getNameAsString())) {
                            methodCallExprs_want.add(callExpr);
                            break;
                        }
                    }
                    for (FieldAccessExpr fieldAccessExpr: fieldAccessExpr_want_done) {
                        if (tmp_fields.contains(fieldAccessExpr.getNameAsString())) {
                            methodCallExprs_want.add(callExpr);
                            break;
                        }
                    }
...
//        });
        ...
        ArrayList<Node> final_nodes_want_end = new ArrayList<Node>();
        final_nodes_want_end.addAll(final_nodes_want);
        // 相同行号的node出现，保留范围最广的node
        for (Node node: final_nodes_want) {
            for (Node node_end: final_nodes_want) {
                if (node != node_end && node_end.getBegin().get().line == node.getBegin().get().line){
                    if (node_end.findAll(Node.class).contains(node)){
                        final_nodes_want_end.remove(node);
                    } else {
                        final_nodes_want_end.remove(node_end);
                    }
                }
            }
        }
//        final_nodes_want.add(methodCallExpr);
        ...
        return final_nodes_want_end;

    }

    public Boolean isVariableBelongClass (int line) {
        MethodDeclaration methodDeclaration = new MethodDeclaration();
        for (MethodDeclaration methodDeclar:this.methodDeclarations) {
            if (methodDeclar.getBegin().get().line<=line && methodDeclar.getEnd().get().line>=line){
                methodDeclaration = methodDeclar;
                break;
            }
        }
        ArrayList<String> classNames = new ArrayList<>();
        classNames.add("File");
        classNames.add("Nonce");
        classNames.add("String");
        classNames.add("URI");
        classNames.add("WebContext");
        List<String> nameExprs = methodDeclaration.findAll(NameExpr.class).stream().filter(nameExpr -> nameExpr.getBegin().get().line==line && !this.SimpleNamesOfClassOrInterfaceType.contains(nameExpr.getName())).map(NameExpr::getNameAsString).collect(Collectors.toList());
        List<String> fieldAccessExprs = methodDeclaration.findAll(FieldAccessExpr.class).stream().filter(fieldAccessExpr -> fieldAccessExpr.getBegin().get().line==line && fieldAccessExpr.getScope().isThisExpr()).map(FieldAccessExpr::getNameAsString).collect(Collectors.toList());
        List<VariableDeclarator> variableDeclarators_of_methodCall = methodDeclaration.findAll(VariableDeclarator.class);
        ArrayList<Parameter> parameters_of_methodDeclaration = new ArrayList<>(methodDeclaration.getParameters());
        for (Parameter parameter : parameters_of_methodDeclaration) {
            if (nameExprs.contains(parameter.getNameAsString()) && classNames.contains(parameter.getType().toString())) {
                System.out.println(parameter.getType().toString());
                System.out.println(parameter.getNameAsString());
                return Boolean.TRUE;}
        }
        for (VariableDeclarator variableDeclarator : variableDeclarators_of_methodCall) {
            if (variableDeclarator.getBegin().get().line == line){
                nameExprs.add(variableDeclarator.getNameAsString());
            }
            if (nameExprs.contains(variableDeclarator.getNameAsString()) && classNames.contains(variableDeclarator.getType().toString())) {
                System.out.println(variableDeclarator.getType().toString());
                System.out.println(variableDeclarator.getNameAsString());
                return Boolean.TRUE;}
        }
        for (VariableDeclarator variableDeclarator : this.field_variableDeclarators) {
            if (variableDeclarator.getBegin().get().line == line){
                fieldAccessExprs.add(variableDeclarator.getNameAsString());
            }
            if (fieldAccessExprs.contains(variableDeclarator.getNameAsString()) && classNames.contains(variableDeclarator.getType().toString())) {
                System.out.println(variableDeclarator.getType().toString());
                System.out.println(variableDeclarator.getNameAsString());
            return Boolean.TRUE;}
        }
...
//            variableDeclarator.setName("unused");
    ...
    public ArrayList<MethodCallExpr> getMethodCallOfline(int line){
        ...
        for (MethodCallExpr methodCallExpr: this.methodCallExprs) {
            if (methodCallExpr.getBegin().get().line == line){
                if(methodCallExpr.getName().toString().contains("set")||
                        methodCallExpr.getName().toString().contains("remove")||
                        methodCallExpr.getName().toString().contains("get")||
                        methodCallExpr.getName().toString().contains("append")||
                        methodCallExpr.getName().toString().contains("add")||
                        methodCallExpr.getName().toString().contains("has")||
                        methodCallExpr.getName().toString().contains("to")||
                        methodCallExpr.getName().toString().contains("delete"))
                methodCallExprsOfline.add(methodCallExpr);
            }
        }
...
//        int want_line= Integer.parseInt(args[1]);
        ...
        Parse_new_4 p = new Parse_new_4(datapath);
...
//--------------------------------------
        ...
        System.out.println(p.isVariableBelongClass(208));
